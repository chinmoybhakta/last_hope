...
class DownloadService {
  ...
  Future<String?> downloadModel({
    ...
    try {
      ...
      // Use native library's expected hardcoded path directly
      final nativeModelsDir = Directory('/data/user/0/com.example.last_hope/app_flutter/models');

      if (!await actualModelsDir.exists()) {
        await actualModelsDir.create(recursive: true);
      }
      
      if (!await nativeModelsDir.exists()) {
        await nativeModelsDir.create(recursive: true);
      }

      // Save to native expected path for direct compatibility
      final filePath = '${nativeModelsDir.path}/$filename';
